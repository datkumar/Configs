...
def get_players():
    out = run_cmd(["playerctl", "-l"])
    return out.splitlines() if out else []

def get_metadata(player):
    title = run_cmd(["playerctl", "-p", player, "metadata", "title"])
    artist = run_cmd(["playerctl", "-p", player, "metadata", "artist"])
    return title, artist


# NOTE: I turned off tooltip in config so ignore that content
...
def main():
    ...
    # Prioritize players in 'Playing' state only
    for player in players:
        status = run_cmd(["playerctl", "-p", player, "status"])
        if status == "Playing":
            title, artist = get_metadata(player)
            if not title:
                continue
            icon = ""
    ...
    # Nothing currently playing, so don't show anything
    print("")
